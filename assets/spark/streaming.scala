...
import com.amazonaws.services.lakeformation.{AWSLakeFormation, AWSLakeFormationClient}
...
import org.apache.spark.sql.streaming.{DataStreamWriter, StreamingQueryException, Trigger}
...
object GlueApp {
  ...
  val s3BasePath = "s3://octank-sdp/"
  val checkpointsFolder = "checkpoints/"
  val checkpointSuffix = "/load_test_checkpoint_with_enrichment_" + ts
  val s3checkpoint = s3BasePath + checkpointsFolder + tableName + checkpointSuffix
  ...
  def main(sysArgs: Array[String]) {
    ...
    val ipDF: DataFrame = spark.read.format("csv")
    .option("header", true)
    .schema(ipSchema)
    .load("s3://octank-sdp/data/enrich_ip_netzone_2021-12-02.csv")
    ...
    val options_read: Map[String, String] = Map(
      "kafka.bootstrap.servers" -> "b-3.msk-cluster-1.e4cubr.c24.kafka.us-east-1.amazonaws.com:9092,b-14.msk-cluster-1.e4cubr.c24.kafka.us-east-1.amazonaws.com:9092,b-4.msk-cluster-1.e4cubr.c24.kafka.us-east-1.amazonaws.com:9092",
      "subscribe" -> "netflow20",
    ...
    data_frame_datasource0.writeStream.foreachBatch(
           (df: Dataset[Row], batchId: Long) => {

      val dataFrameWithAddedColumns: DataFrame = df.withColumn("ts", to_timestamp(col("timestamp")))
    ...
    // ENRICHMENT: internal ipDFCached (netzone)
      val enrichedDataFrame: DataFrame = 
            dataFrameWithAddedColumns.alias("a")
                .join(netzoneDF.alias("b"), col("a.ip_src") === col("b.ip"), "left")
                .select(col("a.*"), col("b.zone").alias("src_zone"), col("b.area").alias("src_area"))

      val joinedDataframe: DataFrame = 
            enrichedDataFrame.alias("a")
                .join(netzoneDF.alias("b"), col("a.ip_dst") === col("b.ip"), "left")
                .select(col("a.*"), col("b.zone").alias("dst_zone"), col("b.area").alias("dst_area"))
      // publish SNS topic        
      joinedDataframe.write.mode("append").partitionBy("year","month", "day", "hour").parquet(s3Output)
