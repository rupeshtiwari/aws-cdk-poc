...
"""
python3 producer.py --count 1 --cycle 1
"""
import argparse
...
# broker information
bserver = 'b-3.msk-cluster-1.e4cubr.c24.kafka.us-east-1.amazonaws.com:9092,b-14.msk-cluster-1.e4cubr.c24.kafka.us-east-1.amazonaws.com:9092,b-4.msk-cluster-1.e4cubr.c24.kafka.us-east-1.amazonaws.com:9092'
...
def parse_arguments():
    ...
    parser.add_argument('--topic-name', dest='topic', action='store',
                        default='netflow20', help='netflow', required=False)
    parser.add_argument('--client-id', dest='clientid', action='store',
                        default='octank_sdp', help='Kafka-P01', required=False)
...
if __name__ == '__main__':
    ...
    print('End of data gen - ',
          str(datetime.datetime.now()),' Total record counts - ' f'{len(messageAr)*cy}')
 

    """
    pipeline can break because too much data it has so flush every 100,000 message do flush
    100k. It creates a pipeline queue which has limitation of having some number of messges
    it can fail. 
    """
    def flush(producer):
        # print (f'Push to kafka: topic={topic}, time={str(datetime.datetime.now())}')
        producer.flush()
